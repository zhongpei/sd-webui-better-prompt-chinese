...
if shared.opts.better_prompt_model_device == "cpu":
    device = "cpu"
    device_id = -1
...
class Models(ModelsBase):
    ...
    def load_gpt_neo_125m(cls):
        return pipeline(
            'text-generation',
            model=shared.opts.better_prompt_model,
            tokenizer=shared.opts.better_prompt_model,
            device=device_id,
    ...
    def load_microsoft_model(cls):
        model = AutoModelForCausalLM.from_pretrained(
            pretrained_model_name_or_path=shared.opts.better_prompt_model,
            trust_remote_code=True,
...
def _generate_prompt(
    ...
    if model_name == 'Ar4ikov/gpt2-650k-stable-diffusion-prompt-generator':
